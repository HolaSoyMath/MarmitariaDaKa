...
import { useEffect, useCallback, useMemo, useState, type Dispatch, type SetStateAction } from 'react'
...
import { getPurchaseItemColumns, type DraftPurchaseRow } from '@/types/columnDefs/purchaseItemColumns'
...
interface PurchaseItemsTabProps {
  ...
  onRowsChange: Dispatch<SetStateAction<DraftPurchaseRow[]>>
...
export function PurchaseItemsTab({
  ...
  const handleChangeRow = useCallback(
    (index: number, field: keyof DraftPurchaseRow, value: string) => {
      onRowsChange((prev) => prev.map((row, i) => (i === index ? { ...row, [field]: value } : row)))
    },
    [onRowsChange],
  )
  ...
  const handleAddRow = () => {
    onRowsChange((prev) => [...prev, { ingredientId: '', unit: '', quantity: '', totalValue: '', location: undefined }])
  }
  ...
  const columns = useMemo(
    () => getPurchaseItemColumns({ ingredients, onChangeRow: handleChangeRow, onRemoveRow: handleRemoveRow }),
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3">
        <Button variant="outline" size="sm" onClick={handleAddRow} type="button" className="gap-1.5">
          <Plus className="size-4" />
          Adicionar item
        </Button>
        <Button variant="ghost" size="sm" onClick={() => setIngredientSheetOpen(true)} type="button" className="gap-1.5">
          <Plus className="size-4" />
          Novo ingrediente
        </Button>
        <Button
          size="sm"
          onClick={handleSave}
          disabled={isSaving || rows.filter((r) => r.ingredientId && r.quantity && r.totalValue).length === 0}
          type="button"
        >
      ...
      <div className="border border-border rounded-lg bg-secondary shadow-sm mt-4.5 max-w-xs flex flex-col gap-1.5 p-5.5">
        <span className="text-sm text-muted-foreground">Custo de ingredientes (semana)</span>
        <div className="font-heading font-extrabold leading-none tracking-tight text-[30px]">
          {formatCurrency(ingredientTotalCents)}
        </div>
        <span className="font-mono text-xs tracking-wide text-muted-foreground">soma das compras acima</span>
      </div>

      <IngredientSheet open={ingredientSheetOpen} onOpenChange={setIngredientSheetOpen} />
    </div>
  )
}
